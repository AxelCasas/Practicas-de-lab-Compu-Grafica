...
void CrearPiramideTriangular()
{
	...
	Mesh* obj1 = new Mesh();
	obj1->CreateMesh(vertices_piramide_triangular, indices_piramide_triangular, 18, 18);
...
void CreaPyraminx() {
	float offset = 1.0f; // Espacio entre pirámides
	for (int y = 0; y < 3; ++y) { // Filas
		for (int x = 0; x < 3 - y; ++x) { // Columnas en cada fila
			for (int z = 0; z < 3 - y; ++z) { // Profundidad en cada fila
				glPushMatrix();
				glTranslatef((x - (2 - y) * 0.5f) * offset, y * offset, (z - (2 - y) * 0.5f) * offset);
...
int main()
{
	...
	CreaPyraminx(); //inidce 5 de Meshlist
	CreateShaders();
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		meshList[5]->RenderMeshGeometry(); //dibuja las figuras geométricas cilindro, cono, pirámide base cuadrangular
